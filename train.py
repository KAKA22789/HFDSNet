...
def _train(model, args):
    ...
    loss_network = LossNetwork(vgg16)
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            loss_p = loss_network(pred_img[0], label_img4) + \
                     loss_network(pred_img[1], label_img2) + \
                     loss_network(pred_img[2], label_img)

            loss = loss_content + 0.1 * loss_fft + 0.01 * loss_p
            loss.backward()
            ...
            iter_p_adder(loss_p.item())

            epoch_pixel_adder(loss_content.item())
            epoch_fft_adder(loss_fft.item())
            epoch_p_adder(loss_p.item())

            if (iter_idx + 1) % args.print_freq == 0:
                print("Time: %7.4f Epoch: %03d Iter: %4d/%4d LR: %.10f Loss content: %7.4f Loss fft: %7.4f Loss p: %7.4f" % (
                    iter_timer.toc(), epoch_idx, iter_idx + 1, max_iter, scheduler.get_lr()[0],
                    iter_pixel_adder.average(),
                    iter_fft_adder.average(),
                    iter_p_adder.average()))
                writer.add_scalar('Pixel Loss', iter_pixel_adder.average(), iter_idx + (epoch_idx - 1) * max_iter)
                writer.add_scalar('FFT Loss', iter_fft_adder.average(), iter_idx + (epoch_idx - 1) * max_iter)
                writer.add_scalar('P Loss', iter_p_adder.average(), iter_idx + (epoch_idx - 1) * max_iter)
                iter_timer.tic()
        ...
        print("EPOCH: %02d\nElapsed time: %4.2f Epoch Pixel Loss: %7.4f Epoch FFT Loss: %7.4f Epoch p Loss: %7.4f" % (
            epoch_idx, epoch_timer.toc(), epoch_pixel_adder.average(), epoch_fft_adder.average(), epoch_p_adder.average()))
